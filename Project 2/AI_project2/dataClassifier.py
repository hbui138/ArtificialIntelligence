...
#inspriration from: https://github.com/anthony-niklas/cs188/blob/master/p5/dataClassifier.py
def enhancedFeatureExtractorDigit(datum):
    ...
    for i in range(len(pixels)):
        ...
        for j in range(1, len(row)):
            if row[j] != 0:
                non_zero += 1
                if first_Left is None or j < first_Left:
                    first_Left = j
                if j <= (len(pixels) + 1) // 2:
                    above_Center += 1
            if row[j] != row[j-1]:
    ...
    for j in range(len(pixels[0])):
        ...
        for i in range(1, len(col)):
            if col[j] != 0:
                non_zero += 1
                if first_Top is None or i < first_Top:
                    first_Top = i
    ...
    width = len(pixels[0]) - (first_Left * 2) if first_Left is not None else 0
    height = len(pixels) - (first_Top * 2) if first_Top is not None else 0
    aspectRatio = float(width) / height if height != 0 else 0
    percentAbove = float(above_Center) / non_zero if non_zero != 0 else 0
    percentRight = float(past_Right) / non_zero if non_zero != 0 else 0

    for n in range(5):
        features[n] = int(breaks > 175)
    for n in range(10):
        features[(n + 1) * 10] = int(aspectRatio < 0.69)
    for n in range(5):
        features[-n] = int(non_zero > 300)
    for n in range(5):
        features[-(n + 1) * 10] = int(percentAbove > 0.35)
    for n in range(1000, 1005):
        features[n] = int(percentRight < 0.27)
...
def readCommand( argv ):
    ...
    if(options.classifier == "mostFrequent"):
        ...
        import minicontest # type: ignore
